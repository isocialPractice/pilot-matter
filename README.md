...
- **Five game modes** - Runway Landing, Flying through Loops, Dead Stick, Cargo Run and Search and Rescue: three or four stages each, each getting harder at exactly the thing it is about, with every stage timed against your own best. A landing is scored on where down the strip it touched, how far off the middle, how hard, and how square; a course past its first stage lays its gates over, so a loop has to be flown at the angle it was laid at
- **Flying with no engine** - a dead stick opens with the lever already useless, and a cargo run that spends the last of its budget ends up in one. The nose sets the airspeed instead of the throttle, so a strip is reached by a glide that was planned rather than a descent that was flown
