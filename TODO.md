...
- [ ] Add a `## Game Modes` level 2 section to this document, directly
...
- [ ] **Runway**: add a runway to the element registry in
...
- [ ] **Runway landing**: treat a touchdown inside the runway bounds as a
  ...
  - From: Game UI/UX
- [ ] **Start Flight**: add a start-state setting choosing the condition a
...
- [ ] **Toggle Runway**: under **Start off flying**, offer a checkbox that
  ...
  - From: Simulator Configuration
- [ ] **Game Modes menu**: add a **Game Modes** entry to both the start
  ...
  **Settings** entries, listing the available modes and selecting one
  - From: Simulator Configuration
- [ ] Add the first 2 **game modes**, each opening in flight over a
  ...
  - **Flying through Loops**: flying the aircraft through a course of loops
    is the objective
  - From: Game Modes
- [ ] Write 6 further **game mode** ideas as todo items under
  `### New Game Modes` in the new `## Game Modes` section of this document
  - From: `version.control = null`
- [ ] Write 2 improvement items for the **Runway Landing** game mode under a
  `#### Runway Landing` header nested in `### Improve Existing Game Modes`
  - From: `version.control = null`
- [ ] Write 5 improvement items for the **Flying through Loops** game mode
